            ...
            for($i=0;$i<$cntFiles;$i++){
                if (isset($_FILES['txtVCF']['name'][$i]) && $_FILES['txtVCF']['name'][$i] != '') {
                    ...
                    if (!empty($tmpFile) && $tmpFile != '') {
                        // Setup our new file path
                        $newFile = TEMP_FOLDER . date("YmdHis") . $_FILES['txtVCF']['name'][$i];
                        if (!file_exists(TEMP_FOLDER)) {
                            mkdir(TEMP_FOLDER, 777);
                        }
                        ...
                        // FULL NAME
                        $leadData['name']           = $data[0]['FN'][0]['value'][0][0];
                        ...
                        if ($leadId) {
                            foreach ($custom_fields as $field) {
                                ...
                                switch($field['slug']){
                                    ...
                                    case "leads_lastname":
                                        $row = $lastname; break;
                                    case "leads_middlename":
                                        $row = $middlename; break;
                                    case "leads_salutation":
                                        $row = $salutation; break;
                                    case "leads_suffix":
                                        $row = $suffix; break;
