...
/*    public function invoices_report_total()
    {
        ...
        if ($months_report != '') {
            if (is_numeric($months_report)) {
                $minus_months = date('Y-m-d', strtotime("-$months_report MONTH"));
                $custom_date_select = '(tblinvoices.date BETWEEN "' . $minus_months . '" AND "' . date('Y-m-d') . '")';
            } else if ($months_report == 'custom') {
                $from_date = to_sql_date($this->input->post('report_from'));
                $to_date   = to_sql_date($this->input->post('report_to'));
                if ($from_date == $to_date) {
        ...
        $total_without_tax          = $this->db->query('SELECT SUM(subtotal) - SUM(discount_total) + SUM(adjustment) as total_without_tax FROM tblinvoices' . $where . '', FALSE)->result_array();
        $discounts                  = $this->db->query('SELECT SUM(discount_total) as discount_total FROM tblinvoices' . $where . '', FALSE)->result_array();
        $adjustments                  = $this->db->query('SELECT SUM(adjustment) as adjustments FROM tblinvoices' . $where . '', FALSE)->result_array();
        $final                      = array();
        ...
        $final['adjustments']       = format_money($adjustments[0]['adjustments'], $currency_symbol);
