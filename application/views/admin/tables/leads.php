...
$aColumns     = array(
    ...
    'lastcontact',
    'dateadded',
    '(SELECT tblcustomfieldsvalues.value FROM tblcustomfieldsvalues WHERE tblcustomfieldsvalues.relid = tblleads.id AND tblcustomfieldsvalues.fieldto = "clients") as client_type'
    );
$sIndexColumn = "id";
$sTable       = 'tblleads';
...
foreach ($custom_fields as $field) {
    ...
    array_push($join, 'LEFT JOIN tblcustomfieldsvalues as ctable_' . $i . ' 
                    ON tblleads.id = ctable_' . $i . '.relid 
...
if (!is_admin()) {
    switch($staff_role){
        case 6: // CHA
            break;
        default: 
                // array_push($aColumns,'tblcustomfieldsvalues.value as sales_office');
                array_push($where, 'AND (assigned =' . $staff_userid . ' OR addedfrom = ' . $staff_userid . ' OR is_public = 1)'); //default    
                array_push($join, 'LEFT JOIN tblcustomfieldsvalues ON tblcustomfieldsvalues.value = "'.$staff_so.'"
                                    AND tblcustomfieldsvalues.fieldto = "sales_office"
                                    AND tblcustomfieldsvalues.relid = tblleads.id');
            break;
    }
    // array_push($where, 'AND (assigned =' . $staff_userid . ' OR addedfrom = ' . $staff_userid . ' OR is_public = 1)'); //default    
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == '1'){
            $_data = '<div class="checkbox" style="display: none;"><input type="checkbox" value="'.$aRow['id'].'"><label></label></div>';
        } else 
        ...
        } else if( ($aColumns[$i] == '1' || $aColumns[$i] == 'tblleads.id') ){
            $_data = $aRow['id'];
            if(!is_admin()){
                $_data = $cnt;
            }
