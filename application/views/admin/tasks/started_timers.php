...
foreach($_started_timers as $timer){
    ...
    if($task){
        ...
        $data .= _l('timer_top_started', strftime(get_current_date_format().' %H:%M', $timer['start_time'])) . ' - <span class="text-success"><br />'._l('task_total_logged_time') .' '. seconds_to_time_format($this->tasks_model->calc_task_total_time($task->id,' AND staff_id='.get_staff_user_id())).'</span>';
